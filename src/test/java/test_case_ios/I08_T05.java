...
public class I08_T05 {
	...
	public void I08_T05_case(IOSDriver<IOSElement> driver) {
		app_func.sleep(driver, 3);
		System.out.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis()) + " " + "TC I08_T05 실행");
		...
		//틀린 OTP 입력 시 AD, OTP(AD, 미래OTP) 사용자 로그인되지 않음 확인
		app_func.login_server(driver, "soosan3", "admin123!", 4, 0);
		if (app_func.login_confirm(driver, "login") == 0) {
			assertTrue(false);
		}
		
		//정상 OTP 입력 시 AD, OTP(AD, 미래OTP) 사용자 로그인됨을 확인
		...
		System.out.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis()) + " " + "TC I08_T05 종료");
	}
}
