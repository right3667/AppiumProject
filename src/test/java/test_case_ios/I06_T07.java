...
public class I06_T07 {
	func_selenium web_func = new func_selenium();
	func_appium_ios app_func = new func_appium_ios();
	
	Date date_now = new Date(System.currentTimeMillis());
	
	String server_ip = "172.16.150.23";
	String server_port = "443";
	String destination = "30.0.0.5";
	
	String otp_type = "Google OTP";
	
	public void I06_T07_case(IOSDriver<IOSElement> driver) {
		app_func.sleep(driver, 3);

		System.out.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis()) + " " + "TC I06_T07 실행");
		...
		if (web_func.external_otp_setting(webdriver, server_ip, "Google OTP") == 0) {
		...
		//틀린 OTP 입력 시 로컬, OTP(내부, google OTP) 사용자 로그인되지 않음 확인
		app_func.login_server(driver, "default_user", "admin123!", 5, 0);
		
		if (app_func.login_confirm(driver, "login") == 0) {
			assertTrue(false);
		}
		
		//정상 OTP 입력 시 로컬, OTP(내부, google OTP) 사용자 로그인됨을 확인
		...
		System.out.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis()) + " " + "TC I06_T07 종료");
	}
}
