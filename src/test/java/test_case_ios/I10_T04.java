...
public class I10_T04 {
	
	func_appium_ios app_func = new func_appium_ios();
	
	Date date_now = new Date(System.currentTimeMillis());
	
	String server_ip = "172.16.150.35";
	String server_port = "443";
	String destination = "30.0.0.5";
	
	public void I10_T04_case(IOSDriver<IOSElement> driver) {
		app_func.sleep(driver, 3);
		System.out.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis()) + " " + "TC I10_T04 실행");
		...
		//틀린 OTP 입력 시 radius, OTP(radius, 모바일) 사용자 로그인되지 않음 확인
		...
		//정상 OTP 입력 시 radius, OTP(radius, 모바일) 사용자 로그인됨을 확인
		...
		System.out.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis()) + " " + "TC I10_T04 종료");
	}
}
